...
# Function to convert sentence to a sequence of numbers
...
class NERPredictor:
    def __init__(self, model_path="best_model.pt"):
        ...
        try:
            # Load checkpoint with weights_only=False (as the model file is trusted)
    ...
    def extract_entities(self, text):
        ...
        # Make prediction for each word individually
        while i < len(words):
            word = words[i]
            if not word.strip():
                i += 1
            ...
            if label.startswith("B-"):
                ...
                entity_type = label.split("-")[1]  # e.g., "B-UNIVERSITE" -> "UNIVERSITE"
                j = i + 1
                ...
                while j < len(words) and (
                    ...
                    (j < len(words) - 1 and next_predicted == f"B-{entity_type}" and self.predict(words[j+1]) == f"I-{entity_type}") or  # New start but has continuation
                    next_predicted == f"B-{entity_type}"  # Adjacent start tag
                ):
                ...
                # In other cases, move to the next word
                i += 1
        ...
        # Post-check for universities - combine university parts that might be joined
        if "universite" in entities and len(entities["universite"]) > 1:
            new_unis = []
            skip_indices = set()
            
            for i_idx, uni1 in enumerate(entities["universite"]): # Renamed i to i_idx to avoid conflict
                if i_idx in skip_indices:
                    continue
                    
                uni1_lower = uni1.lower()
                combined = uni1
                
                for j_idx, uni2 in enumerate(entities["universite"]): # Renamed j to j_idx
                    if i_idx != j_idx and j_idx not in skip_indices:
        ...
        # Similarly, check for departments as well
        if "bolum" in entities and len(entities["bolum"]) > 1:
            new_bolum = []
            skip_indices = set()
            
            for i_idx, bol1 in enumerate(entities["bolum"]): # Renamed i to i_idx
                if i_idx in skip_indices:
                    continue
                ...
                for j_idx, bol2 in enumerate(entities["bolum"]): # Renamed j to j_idx
                    if i_idx != j_idx and j_idx not in skip_indices:
                        ...
                        # If words are close to each other
                        if words_idx1 and words_idx2 and abs(words_idx1[0] - words_idx2[0]) <= 3:
...
class DemoNERPredictor:
    ...
    def predict(self, text):
        ...
        # Number recognition - smarter check involving numerical values
        ...
        # If it doesn't meet any condition, label as "Other"
        return "O"
        ...
        while i < len(words):
            ...
            if i < len(words) - 1:
                ...
                if any(uni in two_words.lower() for uni in self.uni_words):
                    ...
                    if i < len(words) - 2 and not any(uni in words[i+2].lower() for uni in self.uni_words): # Corrected logic
    ...
    def extract_entities(self, text):
        ...
        if "universite" in entities and len(entities["universite"]) > 1:
            ...
            for i_idx, uni1 in enumerate(entities["universite"]): # Renamed i to i_idx
                if i_idx in skip_indices:
                    continue
...
def turkce_kucuk_yap(text): # Function name not translated as per rule
    return text.casefold()

def is_valid_numara(numara): # Function name not translated
    return numara.isdigit()

def try_assign_to_isim_soyisim(entities, value): # Function name not translated
    # If it starts with a capital letter, add to name; if it ends with typical surname suffixes, add to surname
    if value and value[0].isalpha() and value[0].isupper():
        # Simple surname check
        if value.lower().endswith(("oğlu", "gil", "ler", "lar", "glu", "gül", "öz", "kan")):
            entities["soyisim"].append(value)
        else:
            entities["isim"].append(value)
    else:
        entities.setdefault("unknown", []).append(value) # Changed "bilinmiyor" to "unknown"

# Main application
def main():
    print("NER (Named Entity Recognition) Application")
    print("This application identifies entities such as university, department, name, surname, and number in Turkish texts.")
    print("Loading model, please wait...")
    print("\nSample text: murat ostim teknik üniversitesi bilgisayar mühendisliği şimşek  220201013\n")
    ...
    try:
        ...
        print("\nApplication ready! Enter your text (type 'q' to quit):")
        ...
        while True:
            ...
            # Convert to Turkish lowercase (or simply lowercase if function handles it)
            user_input = turkce_kucuk_yap(user_input)
            
            print("\nAnalyzing text...")
            entities = predictor.extract_entities(user_input)
            
            # Number check and correction
            valid_numaralar = []
            bilinmiyorlar = [] # Variable name not translated
            for numara_val in entities["numara"]: # Renamed numara to numara_val to avoid conflict
                if is_valid_numara(numara_val):
                    valid_numaralar.append(numara_val)
                else:
                    yeni_label = predictor.predict(numara_val) # Variable name not translated
                    if yeni_label == "B-ISIM":
                        entities["isim"].append(numara_val)
                    elif yeni_label == "B-SOYISIM":
                        entities["soyisim"].append(numara_val)
                    else:
                        bilinmiyorlar.append(numara_val)
            entities["numara"] = valid_numaralar
            if bilinmiyorlar:
                entities.setdefault("unknown", []).extend(bilinmiyorlar) # Changed "bilinmiyor" to "unknown"
            ...
            # Unknown information
            if entities.get("unknown"): # Changed "bilinmiyor" to "unknown"
                print(f"Unknown: {', '.join(entities['unknown'])}")
            ...
            for word, label in results:
                formatted_label = format_entity_label(label)
